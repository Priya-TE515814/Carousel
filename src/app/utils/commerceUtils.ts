...
export class CommerceUtils implements OnInit {
    ...
    currency_symbols_config: any = this._teService.commerceSettings.currencySymbolConfig;
    ...
    getFormattedCurrency(currencyIso: any, value: any, decimal?: any) {
        if(value){
            var dec = decimal || 3;
            var symbol = this.getCurrencySymbol(currencyIso);
            var formattedPrice = this.getFormattedPrice(currencyIso, value, dec);
            if (currencyIso === 'EUR') {
    ...
    getCurrencySymbol(currencyIso: any) {
        return currencyIso !== ''? this.currency_symbols_config[currencyIso] : currencyIso;
    }
    

    getFormattedPrice(currencyIso: any, value: any, decimal: any) {
        var newVal;
        value = (value !== undefined && isNaN(value) && value.indexOf(',') !== -1) ? value.replace(/,/g, '') : value;

        if (currencyIso === 'EUR') {
            newVal = this.formatMoney((value * 1), decimal, '.', ',');
            newVal = newVal === '0,00' ? '0,01' : newVal;
        } else {
            newVal = this.formatMoney((value * 1), decimal, ',', '.');
            newVal = newVal === '0.00' ? '0.01' : newVal;
        }
    ...
    formatMoney (value: number, places: any, thousand: any, decimal: any) {
        places = !isNaN(places = Math.abs(places)) ? places : 2;
        thousand = thousand || ",";
        decimal = decimal || ".";
        let i = parseInt(Math.abs(+value || 0).toFixed(places), 10) + ""
        let j;
